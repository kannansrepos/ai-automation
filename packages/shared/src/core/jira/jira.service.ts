...
export const createJiraIssue = async (
  ...
  const resolvedIssueType = ((issueType || process.env.JIRA_ISSUE_TYPE || 'Bug').split(',')[0] || '').trim();
  ...
  const attemptCreate = async (type: string): Promise<string> => {
    ...
    // Extract repository from description if present
    let parsedRepository = 'None';
    const repoRegex = /(?:Repository|Repo):?\s*[*_~]?\s*([a-zA-Z0-9_\-\.\/:]+)/i;
    const match = description.match(repoRegex);
    if (match && match[1]) {
      parsedRepository = match[1].trim();
    }

    if (acFieldId) {
      fields[acFieldId] = acceptanceCriteria;
    }
    if (repoFieldId) {
      fields[repoFieldId] = parsedRepository;
    }

    const response = await client.post('/rest/api/2/issue', { fields });
...
export const createJiraSubtask = async (
  ...
  try {
    ...
    console.log(`JIRA Subtask created successfully: ${issueKey} (Parent: ${parentKey})`);
    return issueKey;
  } catch (error: any) {
    console.error(`Error creating JIRA subtask for ${parentKey}:`, error.response?.data || error.message);
    throw new Error(`Failed to create JIRA subtask: ${error.message}`);
  }
};
...
export const addJiraComment = async (
  ...
  const client = getJiraClient();
  try {
    await client.post(`/rest/api/2/issue/${issueKey}/comment`, {
      body: comment,
    });
    console.log(`[Jira] Successfully added comment to issue ${issueKey}`);
  } catch (error: any) {
    console.error(`[Jira] Failed to add comment to issue ${issueKey}:`, error.response?.data || error.message);
